 ...
 *
 *
 * @package    propel.generator.src.Base
 */
abstract class Task implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\AppBundle\\Model\\Map\\TaskTableMap';
    ...
    /**
     * The value for the descryption field.
     *
     * @var        string
     */
    protected $descryption;
    ...
    /**
     * Initializes internal state of AppBundle\Model\Base\Task object.
     */
    ...
    /**
     * Get the [descryption] column value.
     *
     * @return string
     */
     ...
     * @return $this|\AppBundle\Model\Task The current object (for fluent API support)
     */
    ...
    /**
     * Set the value of [descryption] column.
     *
     * @param string $v new value
     * @return $this|\AppBundle\Model\Task The current object (for fluent API support)
     */
    public function setDescryption($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->descryption !== $v) {
            $this->descryption = $v;
            $this->modifiedColumns[TaskTableMap::COL_DESCRYPTION] = true;
        }
    ...
    public function setEndDate($v)
    {
        ...
        if ($this->end_date !== null || $dt !== null) {
            if ($this->end_date === null || $dt === null || $dt->format("Y-m-d") !== $this->end_date->format("Y-m-d")) {
                ...
                $this->modifiedColumns[TaskTableMap::COL_END_DATE] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : TaskTableMap::translateFieldName('Descryption', TableMap::TYPE_PHPNAME, $indexType)];
            $this->descryption = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : TaskTableMap::translateFieldName('StartDate', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->start_date = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : TaskTableMap::translateFieldName('EndDate', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\AppBundle\\Model\\Task'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TaskTableMap::COL_DESCRYPTION)) {
            $modifiedColumns[':p' . $index++]  = 'descryption';
        }
        if ($this->isColumnModified(TaskTableMap::COL_START_DATE)) {
            $modifiedColumns[':p' . $index++]  = 'start_date';
        }
        if ($this->isColumnModified(TaskTableMap::COL_END_DATE)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'name':
                        ...
                        break;
                    case 'descryption':
                        $stmt->bindValue($identifier, $this->descryption, PDO::PARAM_STR);
                        break;
                    case 'start_date':
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\AppBundle\Model\Task
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setDescryption($arr[$keys[2]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\AppBundle\Model\Task The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TaskTableMap::COL_DESCRYPTION)) {
            $criteria->add(TaskTableMap::COL_DESCRYPTION, $this->descryption);
        }
        if ($this->isColumnModified(TaskTableMap::COL_START_DATE)) {
            $criteria->add(TaskTableMap::COL_START_DATE, $this->start_date);
        }
        if ($this->isColumnModified(TaskTableMap::COL_END_DATE)) {
            $criteria->add(TaskTableMap::COL_END_DATE, $this->end_date);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \AppBundle\Model\Task (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setDescryption($this->getDescryption());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \AppBundle\Model\Task Clone of current object.
     * @throws PropelException
     */
